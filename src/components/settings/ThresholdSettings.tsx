...
const DHT_MARGIN_KEY = "dht_threshold_margin_percentage";
const GAS_MARGIN_KEY = "gas_threshold_margin_units";
const DEFAULT_DHT_MARGIN = 5; // Default 5% for temperature/humidity
const DEFAULT_GAS_MARGIN = 100; // Default ±100 units for gas sensors
...
export function ThresholdSettings({ onThresholdsChange }: ThresholdSettingsProps) {
  ...
  const [dhtMargin, setDhtMargin] = useState<number>(DEFAULT_DHT_MARGIN);
  const [gasMargin, setGasMargin] = useState<number>(DEFAULT_GAS_MARGIN);
  ...
  const loadMarginSettings = () => {
    try {
      // Load DHT margin (percentage)
      const savedDht = localStorage.getItem(DHT_MARGIN_KEY);
      if (savedDht) {
        const margin = parseFloat(savedDht);
        if (!isNaN(margin) && margin >= 5 && margin <= 50) {
          setDhtMargin(margin);
        }
      }

      // Load Gas margin (units)
      const savedGas = localStorage.getItem(GAS_MARGIN_KEY);
      if (savedGas) {
        const margin = parseFloat(savedGas);
        if (!isNaN(margin) && margin >= 50 && margin <= 500) {
          setGasMargin(margin);
        }
      }
    } catch (error) {
  ...
  const saveDhtMargin = (margin: number) => {
    try {
      localStorage.setItem(DHT_MARGIN_KEY, margin.toString());
      setDhtMargin(margin);
      toast({
        title: "DHT Margin Updated",
        description: `Temperature/Humidity margin set to ±${margin}%`,
      });
    } catch (error) {
      console.error("Error saving DHT margin:", error);
      toast({
        title: "Error",
        description: "Failed to save DHT margin setting",
        variant: "destructive"
      });
    }
  };

  const saveGasMargin = (margin: number) => {
    try {
      localStorage.setItem(GAS_MARGIN_KEY, margin.toString());
      setGasMargin(margin);
      toast({
        title: "Gas Sensor Margin Updated",
        description: `Gas sensor margin set to ±${margin} units`,
      });
    } catch (error) {
      console.error("Error saving gas margin:", error);
      toast({
        title: "Error",
        description: "Failed to save gas margin setting",
        variant: "destructive"
      });
    }
  };
  ...
  const applyPreset = async (presetId: string) => {
    ...
    try {
      console.log('🎯 ThresholdSettings: Applying preset with DHT margin:', dhtMargin + '% and gas margin:', gasMargin, 'units');
      await thresholdService.applyPreset(presetId, dhtMargin, gasMargin);
      ...
      toast({
        ...
        description: `${presetName} preset applied: DHT ±${dhtMargin}%, Gas ±${gasMargin} units`,
        variant: "default"
      });
  ...
  const calibrateThresholds = async () => {
    ...
    try {
      console.log('🎯 ThresholdSettings: Calibrating with DHT margin:', dhtMargin + '% and gas margin:', gasMargin, 'units');
      const results = await thresholdService.calibrateThresholds(dhtMargin, gasMargin);
      ...
      toast({
        ...
        description: `Thresholds calibrated: DHT ±${dhtMargin}%, Gas ±${gasMargin} units from ${results.length} sensor readings.`,
        variant: "default"
      });
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 bg-card border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* DHT Sensor Margin Configuration */}
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              DHT Sensors (±{dhtMargin}%)
            </Label>
            <div className="space-y-2">
              <input
                type="range"
                ...
                step="1"
                value={dhtMargin}
                onChange={(e) => saveDhtMargin(parseFloat(e.target.value))}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
              />
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>5%</span>
                <span className="font-medium">±{dhtMargin}%</span>
                <span>50%</span>
              </div>
              <p className="text-xs text-muted-foreground">
                Percentage margin for Temperature & Humidity thresholds
              </p>
            </div>
          </div>

          {/* Gas Sensor Margin Configuration */}
          <div className="space-y-2">
            <Label className="text-sm font-medium">
              Gas Sensors (±{gasMargin} units)
            </Label>
            <div className="space-y-2">
              <input
                type="range"
                ...
                step="10"
                value={gasMargin}
                onChange={(e) => saveGasMargin(parseFloat(e.target.value))}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
              />
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>50</span>
                <span className="font-medium">±{gasMargin}</span>
                <span>500</span>
              </div>
              <p className="text-xs text-muted-foreground">
                Fixed unit margin for Air Quality, Alcohol & Smoke sensors
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedPreset} onValueChange={(value) => {
              ...
              <SelectTrigger>
                <SelectValue placeholder="Choose a crop preset..." />
              </SelectTrigger>
              <SelectContent>
                {presets.map((preset) => (
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {isCalibrating ? 'Calibrating...' : 'Auto-Calibrate from Recent Data'}
            </Button>
          </div>
        </div>
...
// Export function to get current threshold margin setting
export function getThresholdMargin(): number {
  try {
    const saved = localStorage.getItem("threshold_margin_percentage");
    if (saved) {
      const margin = parseFloat(saved);
      if (!isNaN(margin) && margin > 0 && margin <= 50) {
        return margin;
      }
    }
  } catch (error) {
    console.error("Error getting threshold margin:", error);
  }
