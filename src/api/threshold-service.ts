...
class ThresholdService {
   ...
   */
  async applyPreset(presetId: string, dhtMarginPercentage: number = 5, gasMarginUnits: number = 100): Promise<void> {
    try {
      console.log('🎛️ Applying preset to current_thresholds table:', presetId);
      console.log('📊 Using DHT margin percentage:', dhtMarginPercentage + '%');
      console.log('📊 Using gas sensor margin units:', gasMarginUnits);
      ...
      // Air Quality (±fixed units margin)
      if (lastReading?.air_quality_mq135) {
        const reading = lastReading.air_quality_mq135;
        const margin = gasMarginUnits;
        console.log(`🌬️ Air Quality calculation: ${reading} ppm ± ${gasMarginUnits} units = ${margin} margin`);
        console.log(`🌬️ Range: ${(reading - margin).toFixed(2)} - ${(reading + margin).toFixed(2)}`);
        updates.push({
      ...
      // Alcohol (±fixed units margin)
      if (lastReading?.alcohol_mq3) {
        const reading = lastReading.alcohol_mq3;
        const margin = gasMarginUnits;
        console.log(`🧪 Alcohol calculation: ${reading} ppm ± ${gasMarginUnits} units = ${margin} margin`);
        console.log(`🧪 Range: ${(reading - margin).toFixed(2)} - ${(reading + margin).toFixed(2)}`);
        updates.push({
      ...
      // Smoke (±fixed units margin)
      if (lastReading?.smoke_mq2) {
        const reading = lastReading.smoke_mq2;
        const margin = gasMarginUnits;
        console.log(`🔥 Smoke calculation: ${reading} ppm ± ${gasMarginUnits} units = ${margin} margin`);
        console.log(`🔥 Range: ${(reading - margin).toFixed(2)} - ${(reading + margin).toFixed(2)}`);
        updates.push({
  ...
  /**
   * Calibrate thresholds based on latest sensor readings with dual margin system
   */
  async calibrateThresholds(
    dhtMarginPercentage: number = 10,
    gasMarginUnits: number = 100
    ...
    try {
      ...
      for (const sensor of sensorMappings) {
        ...
        if (sensor.value !== null && sensor.value !== undefined && !isNaN(sensor.value) && newThresholds[sensor.key]) {
          ...
          let calculatedLow: number;
          let calculatedHigh: number;
          let marginUsed: number;
          
          // Use dual margin system - DHT sensors get percentage, gas sensors get fixed units
          if (sensor.type === 'air_temperature' || sensor.type === 'air_humidity') {
            // DHT sensors: use percentage margin
            const margin = (currentValue * dhtMarginPercentage) / 100;
            calculatedLow = Math.max(threshold.min, currentValue - margin);
            calculatedHigh = Math.min(threshold.max, currentValue + margin);
            marginUsed = dhtMarginPercentage;
            console.log(`🌡️ DHT sensor ${sensor.type}: ${currentValue} ± ${dhtMarginPercentage}% = ±${margin.toFixed(2)}`);
          } else {
            // Gas sensors: use fixed unit margin
            calculatedLow = Math.max(threshold.min, currentValue - gasMarginUnits);
            calculatedHigh = Math.min(threshold.max, currentValue + gasMarginUnits);
            marginUsed = gasMarginUnits;
            console.log(`🌬️ Gas sensor ${sensor.type}: ${currentValue} ± ${gasMarginUnits} units`);
          }
