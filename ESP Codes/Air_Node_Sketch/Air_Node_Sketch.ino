...
void readSensors();
void sendSensorData();
void testSupabaseConnection();  // New function to test connection
void registerNodeMetadata();    // New function to register node
void testJSONPayload();         // New function to test payload format
void updateDisplay();
...
void handleTestConnection();  // New test connection handler
...
void setup() {
  ...
  // Test Supabase connection during setup
  delay(2000);  // Give WiFi a moment to stabilize
  testSupabaseConnection();
  
  // Register this node in the metadata table
...
void setupWebServer() {
  ...
  server.on("/test-connection", handleTestConnection);     // New test connection button
...
void sendSensorData() {
  ...
  const int RETRY_DELAY = 2000;  // 2 seconds between retries
  
  for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    Serial.printf("🔄 Attempt %d/%d to send data\n", attempt, MAX_RETRIES);
    
    HTTPClient http;
    String serverPath = String(SUPABASE_URL) + String(SUPABASE_ENDPOINT);

    if (attempt == 1) {  // Only log details on first attempt to reduce clutter
      Serial.printf("🔗 Connecting to: %s\n", serverPath.c_str());
      Serial.printf("🔑 Using API key: %s...%s\n", 
        String(SUPABASE_ANON_KEY).substring(0, 10).c_str(),
        String(SUPABASE_ANON_KEY).substring(strlen(SUPABASE_ANON_KEY) - 10).c_str());
    }

    // Create JSON payload
    StaticJsonDocument<512> doc;
    doc["node_id"] = "air_node_01";  // Added required node_id field
    doc["temperature"] = currentSensors.temperature;  // Fixed column name
    doc["humidity"] = currentSensors.humidity;  // Fixed column name
    doc["air_quality_mq135"] = currentSensors.airQuality;  // Fixed column name
    doc["alcohol_mq3"] = currentSensors.alcohol;  // Fixed column name
    doc["smoke_mq2"] = currentSensors.smoke;  // Fixed column name
    
    // Add data validation
    if (isnan(currentSensors.temperature)) {
      doc["temperature"] = nullptr;  // Send null for invalid readings
    }
    if (isnan(currentSensors.humidity)) {
      doc["humidity"] = nullptr;  // Send null for invalid readings
    }
    ...
    http.addHeader("User-Agent", "ESP32-FarmInsight/1.0");
    ...
    if (httpResponseCode > 0) {
      ...
      if (httpResponseCode == 200 || httpResponseCode == 201) {
        Serial.printf("✅ Data sent successfully (HTTP %d) on attempt %d\n", httpResponseCode, attempt);
        if (response.length() < 200) {  // Only show short responses
          Serial.printf("📥 Response: %s\n", response.c_str());
        }
        http.end();
        return;  // Success! Exit retry loop
      } else {
        Serial.printf("⚠️  Unexpected response (HTTP %d) on attempt %d\n", httpResponseCode, attempt);
        Serial.printf("📥 Full Response: %s\n", response.c_str());  // Show full response for debugging
        
        // Parse specific error types
        if (httpResponseCode == 400) {
          Serial.println("🔍 HTTP 400 Bad Request - Common causes:");
          Serial.println("   1. JSON structure doesn't match table schema");
          Serial.println("   2. Missing required fields");
          Serial.println("   3. Data type mismatch (string vs number)");
          Serial.println("   4. Invalid column names");
          Serial.println("   5. Constraint violations");
          
          // Try to extract error details from response
          if (response.indexOf("error") != -1) {
            Serial.println("🔍 Error details found in response:");
            Serial.println(response);
          }
        } else if (httpResponseCode == 401) {
          Serial.println("🔑 HTTP 401 Unauthorized - Check API key");
        } else if (httpResponseCode == 403) {
          Serial.println("🚫 HTTP 403 Forbidden - Check RLS policies");
        } else if (httpResponseCode == 404) {
          Serial.println("📍 HTTP 404 Not Found - Check table/endpoint name");
        }
      }
    } else {
      Serial.printf("❌ HTTP error on attempt %d: %s\n", attempt, http.errorToString(httpResponseCode).c_str());
      Serial.printf("❌ Error code: %d\n", httpResponseCode);
      
      // Additional debugging on first attempt
      if (attempt == 1) {
        if (httpResponseCode == HTTPC_ERROR_CONNECTION_REFUSED) {
          Serial.println("🔍 Troubleshooting tips:");
          Serial.println("   1. Check if Supabase URL is correct");
          Serial.println("   2. Verify internet connectivity");
          Serial.println("   3. Check if firewall is blocking HTTPS");
          Serial.println("   4. Ensure Supabase project is active");
          Serial.println("   5. Verify Supabase API key is correct");
        } else if (httpResponseCode == HTTPC_ERROR_CONNECTION_LOST) {
          Serial.println("🔍 Connection lost - WiFi issue or timeout");
        } else if (httpResponseCode == HTTPC_ERROR_NO_HTTP_SERVER) {
          Serial.println("🔍 No HTTP server found - check URL");
        }
      }
    }
    ...
    if (attempt < MAX_RETRIES) {
      Serial.printf("⏱️  Waiting %d seconds before retry...\n", RETRY_DELAY / 1000);
      delay(RETRY_DELAY);
    }
  }
  
  Serial.printf("❌ All %d attempts failed. Data will be cached locally.\n", MAX_RETRIES);
}
...
void testSupabaseConnection() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("❌ WiFi not connected, cannot test Supabase");
    return;
  }
  ...
  // Test with a simple GET request first (to check if server is reachable)
  http.begin(serverPath);
  http.setTimeout(15000);  // 15 second timeout for test
  http.addHeader("apikey", SUPABASE_ANON_KEY);
  http.addHeader("Authorization", String("Bearer ") + SUPABASE_ANON_KEY);
  http.addHeader("Accept", "application/json");
  ...
  if (httpResponseCode > 0) {
    Serial.printf("✅ Server reachable! HTTP %d\n", httpResponseCode);
    String response = http.getString();
    ...
    if (httpResponseCode == 200) {
      Serial.println("🎉 Supabase connection test PASSED!");
    } else if (httpResponseCode == 401) {
      Serial.println("🔑 Authentication issue - check API key");
    } else if (httpResponseCode == 404) {
      Serial.println("📍 Endpoint not found - check table name");
    }
  } else {
    ...
    if (httpResponseCode == HTTPC_ERROR_CONNECTION_REFUSED) {
      ...
      Serial.println("   - Check if Supabase project URL is correct");
      Serial.println("   - Verify internet connectivity");
      Serial.println("   - Try accessing URL in browser");
    }
  }
...
void registerNodeMetadata() {
  Serial.println("📝 Registering node metadata...");
  
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("❌ WiFi not connected, cannot register node");
    return;
  }
  
  HTTPClient http;
  String serverPath = String(SUPABASE_URL) + "/rest/v1/node_metadata";
  
  // Create JSON payload for node registration
  StaticJsonDocument<512> doc;
  doc["node_id"] = "air_node_01";
  doc["node_name"] = "Farm Insight Garden - Air Quality Node";
  doc["location"] = "Farm Insight Garden";
  
  String jsonPayload;
  serializeJson(doc, jsonPayload);
  
  Serial.printf("📦 Node registration payload: %s\n", jsonPayload.c_str());
  ...
  http.addHeader("apikey", SUPABASE_ANON_KEY);
  http.addHeader("Authorization", String("Bearer ") + SUPABASE_ANON_KEY);
  http.addHeader("Prefer", "resolution=merge-duplicates");  // Update if exists
  
  Serial.println("📡 Sending node registration...");
  int httpResponseCode = http.POST(jsonPayload);
  
  if (httpResponseCode > 0) {
    String response = http.getString();
    if (httpResponseCode == 200 || httpResponseCode == 201) {
      Serial.printf("✅ Node registered successfully (HTTP %d)\n", httpResponseCode);
    } else {
      Serial.printf("⚠️  Node registration response (HTTP %d): %s\n", httpResponseCode, response.c_str());
      
      // If node registration fails, try with upsert to update existing node
      if (httpResponseCode == 409 || httpResponseCode == 400) {
        Serial.println("🔄 Trying upsert mode for existing node...");
        ...
        int upsertResponse = http.POST(jsonPayload);
        if (upsertResponse == 200 || upsertResponse == 201 || upsertResponse == 204) {
          Serial.printf("✅ Node upserted successfully (HTTP %d)\n", upsertResponse);
        } else {
          Serial.printf("❌ Node upsert also failed (HTTP %d)\n", upsertResponse);
        }
        http.end();
      }
    }
  } else {
    Serial.printf("❌ Node registration failed: %s\n", http.errorToString(httpResponseCode).c_str());
  }
  
  http.end();
  Serial.println("📝 Node registration completed\n");
}
...
void testJSONPayload() {
  Serial.println("🧪 Testing JSON payload format...");
  
  // Create the same payload that sendSensorData() creates
  StaticJsonDocument<512> doc;
  doc["node_id"] = "air_node_01";
  ...
  Serial.println("🔍 JSON Payload Analysis:");
  Serial.printf("📦 Raw JSON: %s\n", jsonPayload.c_str());
  Serial.printf("📏 Payload size: %d bytes\n", jsonPayload.length());
  
  // Validate individual fields
  Serial.println("🔍 Field validation:");
  Serial.printf("   node_id: %s (type: string)\n", doc["node_id"].as<String>().c_str());
  Serial.printf("   temperature: %.2f (type: %s)\n", 
    doc["temperature"].as<float>(), 
    doc["temperature"].isNull() ? "null" : "number");
  Serial.printf("   humidity: %.2f (type: %s)\n", 
    doc["humidity"].as<float>(), 
    doc["humidity"].isNull() ? "null" : "number");
  Serial.printf("   air_quality_mq135: %d (type: number)\n", doc["air_quality_mq135"].as<int>());
  Serial.printf("   alcohol_mq3: %d (type: number)\n", doc["alcohol_mq3"].as<int>());
  Serial.printf("   smoke_mq2: %d (type: number)\n", doc["smoke_mq2"].as<int>());
  
  // Check for potential issues
  Serial.println("🔍 Potential issues:");
  if (isnan(currentSensors.temperature) || currentSensors.temperature == 0.0) {
    Serial.println("   ⚠️ Temperature reading invalid (0.0 or NaN)");
  }
  if (isnan(currentSensors.humidity) || currentSensors.humidity == 0.0) {
    Serial.println("   ⚠️ Humidity reading invalid (0.0 or NaN)");
  }
  if (currentSensors.airQuality < 0 || currentSensors.airQuality > 4095) {
    Serial.println("   ⚠️ Air quality reading out of range");
  }
  
  Serial.println("🧪 JSON payload test completed\n");
}
...
void handleRoot() {
  String html = "<!DOCTYPE html>\n"
    ...
    "        </a>\n"
    "        <a href=\"/test-connection\" class=\"btn\" style=\"background: #28a745;\">\n"
...
void handleTestConnection() {
  Serial.println("🧪 Manual database connection test triggered...");
  ...
  String html = "<!DOCTYPE html>\n"
    ...
    "    pre { background: #f8f9fa; padding: 10px; border-radius: 5px; text-align: left; overflow-x: auto; }\n"
    "  </style>\n"
    "</head>\n"
    "<body>\n"
    "  <div class=\"container\">\n"
    ...
    "      <pre>\n"
    "Node ID: air_node_01\n"
    "Temperature: " + String(currentSensors.temperature, 1) + "°C\n"
    "Humidity: " + String(currentSensors.humidity, 0) + "%\n"
    "Air Quality: " + String(currentSensors.airQuality) + "\n"
    "Alcohol: " + String(currentSensors.alcohol) + "\n"
    "Smoke: " + String(currentSensors.smoke) + "\n"
    "      </pre>\n"
    "    </div>\n"
    "    \n"
    "    <p>🔍 Check the Serial Monitor (115200 baud) for detailed test results.</p>\n"
  ...
  // Run the connection test after sending response
  delay(500);
  testSupabaseConnection();
  
  // Test the JSON payload format
  testJSONPayload();
  
  // Try to send current sensor data as a test
  Serial.println("🧪 Testing sensor data transmission...");
  sendSensorData();
}
